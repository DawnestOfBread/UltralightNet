...
{
	/// <summary>
	///     The View will be rendered to an offscreen GPU texture using the <see cref="UlPlatform.GPUDriver" />. You can
	///     fetch details for the texture via <see cref="View.RenderTarget" />.
	/// </summary>
	...
	{
		return IsAccelerated == other.IsAccelerated &&
		       IsTransparent == other.IsTransparent &&
		       Math.Abs(InitialDeviceScale - other.InitialDeviceScale) < double.Epsilon &&
	...
	{
		return obj is UlViewConfig ? Equals((UlViewConfig)obj) : false;
	}
	#if NETSTANDARD2_1 || NETCOREAPP2_1_OR_GREATER
	public readonly override int GetHashCode()
	{
		return HashCode.Combine(
			HashCode.Combine(IsAccelerated, IsTransparent, InitialDeviceScale, InitialFocus, EnableImages,
				EnableJavaScript, FontFamilyStandard, FontFamilyFixed),
			HashCode.Combine(FontFamilySerif, FontFamilySansSerif, UserAgent));
	}
	#else
